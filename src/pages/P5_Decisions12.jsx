...
// The component.
const P5_Decisions12 = (props) => {
    // Mandatory definitions.
    const { t, i18n } = useTranslation();
    const { parameter, c, relativeDate } = useContext(globalContext);

    const offset = parameter.exchangeRates.length * 2  // number of previous decisions.
    ...
    return (
        <Container>
            ...
            <Text>{t("decisions-12-text-1", translationProps)}</Text>

            <Calendar
                startDate={date1.dateObject}
                endDate={date2.dateObject}
            />

            <Text>{t("decisions-12-text-2", translationProps)}</Text>

            <Form
                name="decisions-12-form"
            ...
            >
                {
                    parameter.exchangeRates.map((exchangeRate, index) =>
                        <CTBSlider
                            key={index}
                            name={`decision_12_${exchangeRate}`}
