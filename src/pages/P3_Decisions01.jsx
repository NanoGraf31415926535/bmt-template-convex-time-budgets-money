...
// The component.
const P3_Decisions01 = (props) => {
    // Mandatory definitions.
    const { t, i18n } = useTranslation();
    const { parameter, validation, c, n, relativeDate } = useContext(globalContext);

    const offset = 0  // number of previous decisions.
    ...
    return (
        <Container>
            ...
            <Text>{t("decisions-01-text-1", translationProps)}</Text>

            <Calendar
                startDate={date0.dateObject}
                endDate={date1.dateObject}
            />

            <Text>{t("decisions-01-text-2", translationProps)}</Text>

            <Form
                name="decisions-01-form"
            ...
            >
                {
                    parameter.exchangeRates.map((exchangeRate, index) =>
                        <CTBSlider
                            key={index}
                            name={`decision_01_${exchangeRate}`}
